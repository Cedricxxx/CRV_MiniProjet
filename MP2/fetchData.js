const URL = process.env.URL || 'http://10.106.10.63:8080' //const URL = process.env.URL || 'http://localhost:8080'
...
const writeAndRead = async (max = 10000, iter = 10) => {
  ...
  while (call < max) {
    ...
    const writeRes = Promise.all(
      new Array(Math.floor(iter/10)).fill(1).map((_) => { //new Array(Math.floor(iter/10)).fill(1).map((_) => { 
